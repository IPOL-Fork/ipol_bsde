...
dbRGB operator+(const dbRGB &p1, const dbRGB &p2)
{
    return dbRGB(p1.R+p2.R,p1.G+p2.G,p1.B+p2.B);
}
dbRGB operator-(const dbRGB &p1, const dbRGB &p2)
{
    return dbRGB(p1.R-p2.R,p1.G-p2.G,p1.B-p2.B);
...
float dbRGB::abs(dbRGB p)
{
    return sqrtf(p.R*p.R+p.G*p.G+p.B*p.B);
}
dbRGB dbRGB::round(dbRGB p)
{
    return dbRGB(std::round(p.R),std::round(p.G),std::round(p.B));
}
dbRGB dbRGB::max(float p1, dbRGB p2)
{
    return dbRGB(std::fmax(p1,p2.R),std::fmax(p1,p2.G),std::fmax(p1,p2.B));
}
dbRGB dbRGB::min(float p1, dbRGB p2)
{
    return dbRGB(std::fmin(p1,p2.R),std::fmin(p1,p2.G),std::fmin(p1,p2.B));
}
...
size_t dbImage::getWidth()
{
    return Width;
}
size_t dbImage::getHeight()
{
    return Height;
}
float* dbImage::getArray()
{
    return A;
}
dbImage::dbImage()
...
dbImage::dbImage(size_t W, size_t H)
{
    Width = W;
    Height = H;
    A = new float[W*H];
}
dbImage::dbImage(float* T,size_t W, size_t H)
{
    A=T;
    Width=W;
    Height=H;
...
size_t dbImageRGB::getWidth()
{
    return Width;
}
size_t dbImageRGB::getHeight()
{
    return Height;
}
dbRGB* dbImageRGB::getArray()
{
    return A;
}
...
dbImageRGB::dbImageRGB(size_t W, size_t H)
{
    Width = W;
    Height = H;
    A = new dbRGB[W*H];
}
dbImageRGB::dbImageRGB(dbRGB* T,size_t W, size_t H)
{
    A=T;
    Width=W;
    Height=H;
